    ...
    useEffect(() => {
        (async () => {
            const snapshot = await db.collection('crm_leads')
                .where('user', '==', db.collection('users').doc(user?.id))
                .orderBy('createdAt', 'desc')
                .get();
            setLeads(snapshot.docs.map(lead => lead.data()));
        })();
    }, []);
    ...
    return <div class="p-4">
        ...
        <div class="text-sm font-medium text-center text-body border-b border-default">
            <ul class="flex flex-wrap -mb-px" data-tabs-toggle="#default-styled-tab-content" data-tabs-active-classes="text-purple hover:text-purple border-purple" data-tabs-inactive-classes="dark:border-transparent text-body hover:text-fg-brand border-default hover:border-brand">
                <li class="me-2">
                    <a href="#" class="inline-block p-4 border-b border-transparent rounded-t-base hover:text-fg-brand hover:border-brand" aria-current="page">Leads</a>
                </li>
                <li class="me-2">
                    <a href="#" class="inline-block p-4 text-fg-brand border-b border-brand rounded-t-base active">Contact</a>
                </li>
                <li class="me-2">
                    <a href="#" class="inline-block p-4 border-b border-transparent rounded-t-base hover:text-fg-brand hover:border-brand">Interest</a>
                </li>
                <li class="me-2">
                    <a href="#" class="inline-block p-4 border-b border-transparent rounded-t-base hover:text-fg-brand hover:border-brand">Considering</a>
                </li>
                <li>
                    <a class="inline-block p-4 text-fg-disabled rounded-t-base cursor-not-allowed dark:text-body">Converted</a>
                </li>
                <li>
                    <a class="inline-block p-4 text-fg-disabled rounded-t-base cursor-not-allowed dark:text-body">Churn</a>
                </li>
            </ul>
        </div>
        ...
        <div>
            ...
            <div id="dropdown-2" class="z-10 text-white hidden bg-neutral-primary-medium border border-default-medium rounded-base shadow-lg w-32">
            ...
            {leads && leads.length > 0 &&
                <div class="relative overflow-x-auto bg-neutral-primary-soft shadow-xs rounded-base border border-default">
                    <table class="w-full text-sm text-left rtl:text-right text-body">
                        ...
                        <tbody>
                            {leads.map(u => <tr class="bg-neutral-primary-soft border-b border-default hover:bg-neutral-secondary-medium">
                                <td class="w-4 p-4">
                                    <div class="flex items-center">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="h-2.5 w-2.5 rounded-full bg-success me-2"></div> {u?.createdAt?.toDate ? formatDateTime(u?.createdAt.toDate()) : u?.createdAt}
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="h-2.5 w-2.5 rounded-full bg-success me-2"></div> {u?.updatedAt?.toDate ? formatDateTime(u?.updatedAt.toDate()) : u?.updatedAt}
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
